    ...
    pub fn on_frame_advance(
        ...
        for row in rows {
            ...
            let note = if graduated {
                // magnitude = node size (cluster_value can also be used)
                graduated_note(base, row.size, grad_config)
            } else {
                base
            };
            let instrument = row.instrument;
            let vel = (row.velocity as u32).min(self.velocity as u32) as u8;
...
mod tests {
    ...
    fn test_beats_fire_at_slice_0() {
        ...
        sched.on_frame_advance(&dummy_frame(0), &rows, false, &grad_cfg());
        ...
        sched.on_frame_advance(&dummy_frame(1), &rows, false, &grad_cfg());
    ...
    fn test_beats_fire_two_per_transition() {
        ...
        for slice in 0u32..10 {
            ...
            sched.on_frame_advance(&dummy_frame(slice), &rows, false, &grad);
